...
double CalculateValidChildrenWidth(FrameworkElement element, int*& childrenCount) {
  ...
  for (int i = 0; i < *childrenCount; i++) {
    ...
    if (className == L"Taskbar.TaskListButton" || className == L"Taskbar.ExperienceToggleButton") {
      SetElementPropertyFromString(child, className.c_str(), L"CornerRadius", L"14");
    } else {
...
bool ApplyStyle(XamlRoot xamlRoot) {
  ...
  double childrenWidth = CalculateValidChildrenWidth(taskbarFrameRepeater, childrenCount);
  ...
  float newXOffset = (centered + childrenWidth / 2) + Wh_GetIntSetting(L"Tray task gap");

  boolean movingInwards = originalOffset.x > newXOffset;

  Wh_Log(L"Original Offset: %f, New offset: %f, Children %f", originalOffset.x, newXOffset, childrenWidth);
  ...
  if (movingInwards) {
    trayAnimation.DelayTime(winrt::Windows::Foundation::TimeSpan(std::chrono::milliseconds(*childrenCount * 4)));
  }
  auto batchTray = compositor.CreateScopedBatch(winrt::Windows::UI::Composition::CompositionBatchTypes::Animation);
  ...
  auto taskbarStroke = FindChildByName(FindChildByClassName(FindChildByClassName(rootGrid, L"Taskbar.TaskbarBackground"), L"Windows.UI.Xaml.Controls.Grid"), L"BackgroundStroke");
  taskbarStroke.Opacity(0);

  auto backgroundFillParent = FindChildByClassName(FindChildByClassName(rootGrid, L"Taskbar.TaskbarBackground"), L"Windows.UI.Xaml.Controls.Grid");
  auto backgroundFillChild = FindChildByName(backgroundFillParent, L"BackgroundFill");

  if (backgroundFillChild) {
    SetElementPropertyFromString(backgroundFillParent, L"Windows.UI.Xaml.Controls.Grid", L"CornerRadius", L"20");
    ...
    float targetWidth = g_unloading ? rootWidth : (childrenWidth + trayFrameWidth);
    ...
    roundedRect.CornerRadius({20.0f, 20.0f});
    roundedRect.Size({targetWidth, static_cast<float>(backgroundFillChild.ActualHeight())});
    ...
    sizeAnimationRect.InsertKeyFrame(0.0f, {g_lastTargetWidth, static_cast<float>(backgroundFillChild.ActualHeight())});  // Initial size
    sizeAnimationRect.InsertKeyFrame(1.0f, {targetWidth, static_cast<float>(backgroundFillChild.ActualHeight())});        // Target size
    ...
    offsetAnimationRect.InsertKeyFrame(0.0f, {g_lastTargetOffsetX, 0.0f});  // Initial offset
    offsetAnimationRect.InsertKeyFrame(1.0f, {targetOffsetX, 0.0f});        // Target offset
